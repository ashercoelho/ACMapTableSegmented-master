...
- (void)layoutSubviews {
    ...
    titleFrame.origin.x = ceil(remainingWidth*position+22);
    titleFrame.origin.y = 19;
    ...
    arrowFrame.origin.x = ceil(remainingWidth*position-12);
    ...
    if(hasCustomView) {
        ...
        //cast para evitar erro de duplicação do método nas classes
        UIView *customViewAfterCast = (UIView *)customView;
        CGRect viewBounds = [customViewAfterCast bounds];
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"Puxe para atualizar...",);
